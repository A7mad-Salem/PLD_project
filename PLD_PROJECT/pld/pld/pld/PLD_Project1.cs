...
namespace com.calitha.goldparser
{
    ...
    enum RuleConstants : int
    {
        ...
        RULE_EXP                                 = 24, // <exp> ::= <id>
        RULE_EXP_QUOTE_QUOTE                     = 25, // <exp> ::= '"' <id> '"'
        RULE_EXP_NUMBER                          = 26, // <exp> ::= Number
        RULE_IF_STMT_IF_COLON                    = 27, // <if_stmt> ::= if <cond> ':' <stmt_list>
        RULE_IF_STMT_IF_COLON_ELSE               = 28, // <if_stmt> ::= if <cond> ':' <stmt_list> else <stmt_list>
        RULE_COND                                = 29, // <cond> ::= <expr> <opr> <expr>
        RULE_COND_TRUE                           = 30, // <cond> ::= TRUE
        RULE_COND_FALSE                          = 31, // <cond> ::= FALSE
        RULE_OPR_GT                              = 32, // <opr> ::= '>'
        RULE_OPR_EQEQ                            = 33, // <opr> ::= '=='
        RULE_OPR_LT                              = 34, // <opr> ::= '<'
        RULE_OPR_EXCLAMEQ                        = 35, // <opr> ::= '!='
        RULE_OPR_LTEQ                            = 36, // <opr> ::= '<='
        RULE_OPR_GTEQ                            = 37, // <opr> ::= '>='
        RULE_FOR_STMT_FOR_IN_COLON               = 38, // <for_stmt> ::= for <id> in <expr> ':' <stmt_list>
        RULE_WHILE_STMT_WHILE_COLON              = 39, // <while_stmt> ::= while <expr> ':' <stmt_list>
        RULE_DOWHILE_STMT_DO_LPAREN_RPAREN_WHILE = 40, // <do-while_stmt> ::= do '(' <stmt_list> ')' while <cond>
        RULE_SWITCH_STMT_SWITCH_COLON            = 41, // <switch_stmt> ::= switch <expr> ':' <nl> <stmt_list>
        RULE_SWITCH_CASES                        = 42, // <switch_cases> ::= <switch_case> <expr>
        RULE_SWITCH_CASES2                       = 43, // <switch_cases> ::= <switch_case> <nl> <switch_cases>
        RULE_SWITCH_CASE_CASE_COLON              = 44, // <switch_case> ::= case <expr> ':' <nl> <stmt_list>
        RULE_SWITCH_CASE                         = 45, // <switch_case> ::= <default>
        RULE_DEFAULT_DEFAULT                     = 46, // <default> ::= Default
        RULE_PRINT_STMT_PRINT_LPAREN             = 47, // <print_stmt> ::= print '(' <expr>
        RULE_PRINT_STMT_QUOTE_QUOTE_RPAREN       = 48  // <print_stmt> ::= '"' <expr> '"' ')'
    };
    ...
    public class MyParser
    {
        ...
        public Object CreateObjectFromNonterminal(NonterminalToken token)
        {
            switch (token.Rule.Id)
            {
                ...
                case (int)RuleConstants.RULE_EXP_QUOTE_QUOTE :
                //<exp> ::= '"' <id> '"'
                //todo: Create a new object using the stored tokens.
                return null;
